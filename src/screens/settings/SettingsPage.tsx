...
// Placeholder Styled Checkbox Toggle
const ToggleSwitch = ({ id, label, checked, onChange }: { id: string; label: string; checked: boolean; onChange: (checked: boolean) => void }) => {
  return (
    <label htmlFor={id} className="flex items-center cursor-pointer">
      ...
      <span className="ml-3 text-gray-700 dark:text-gray-300">{label}</span> {/* Label moved outside for clarity */}
    </label>
  );
};
...
export const SettingsPage = (): JSX.Element | null => {
  ...
  // Restore local state for toggles
  ...
  // Handle dropdown change and update context theme
        ...
        <section className="mb-10">
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <ToggleSwitch id="soundEffects" label="" checked={soundEffects} onChange={setSoundEffects} />
            </div>
             <div className="flex justify-between items-center">
